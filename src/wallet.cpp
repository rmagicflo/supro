...
/**
 * Fees smaller than this (in usp) are considered zero fee (for transaction creation)
...
CAmount CWalletTx::GetUnlockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && (vout[i].nValue == GetMNCollateral(chainActive.Height()) * COIN || vout[i].nValue == GetMNCollateralNew(chainActive.Height()) * COIN)) continue; // do not count MN-like outputs
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        else if (fMasterNode && (vout[i].nValue == GetMNCollateral(chainActive.Height()) * COIN || vout[i].nValue == GetMNCollateralNew(chainActive.Height()) * COIN)) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_NOT10000IFMN) {
                    found = !(fMasterNode && (pcoin->vout[i].nValue == GetMNCollateral(chainActive.Height()) * COIN || pcoin->vout[i].nValue == GetMNCollateralNew(chainActive.Height()) * COIN));
                } if (nCoinType == ONLY_10000) {
                    found = pcoin->vout[i].nValue == GetMNCollateral(chainActive.Height()) * COIN || pcoin->vout[i].nValue == GetMNCollateralNew(chainActive.Height()) * COIN;
                } else {
                    found = true;
                }
                if (!found) continue;
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 SP.");
                    }
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-sp-address
                    CScript scriptChange;
