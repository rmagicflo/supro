...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest) {
...

        {
            ...
            if (!AcceptableInputs(mempool, state, CTransaction(tx), false, NULL) && chainActive.Height() > 235500) {
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    //     if (addr.GetPort() != 47113) return false;
    // } else if (addr.GetPort() == 47113)
    //     return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when SP collateral tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
