...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SuproMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("SuproMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SuproMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("supro-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            if (mapHashedBlocks.count(chainActive.Tip()->nHeight)) { //search our map of hashed blocks, see if bestblock has been hashed yet
                if (GetTime() - mapHashedBlocks[chainActive.Tip()->nHeight] < max(pwallet->nHashInterval, (unsigned int)1)) { // wait half of the nHashDrift with max wait of 5 minutes
                    MilliSleep(5000);
                    continue;
                }
            }
        ...
        LogPrintf("Running SuproMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
